...
PREVIOUS_PATCH_WIN = 'UWR_Patch_10_6_23_4_27'
...
DENOISER_MODEL_PATH = os.path.join(VR_MODELS_DIR, 'UWR-DeNoise-Lite.pth')
DEVERBER_MODEL_PATH = os.path.join(VR_MODELS_DIR, 'UWR-DeEcho-DeReverb.pth')
...
class ModelData():
    ...
    def get_demucs_model_data(self):
        ...
        if DEMUCS_UWR_MODEL in self.model_name:
...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def menu_help(self):#**
        ...
        credit_label(place=12,
                     frame=credits_Frame,
                     text="Bas Curtiz",
                     message="Designed the official UWR logo, icon, banner, splash screen.")
        
        credit_label(place=14,
                     frame=credits_Frame,
                     text="DilanBoskan",
                     message="Your contributions at the start of this project were essential to the success of UWR. Thank you!")
        ...
        Link(place=18, 
             ...
             description="UWR relies on FFmpeg for processing non-wav audio files.\n" + \
        ...
        Link(place=22, 
             ...
             description="UWR uses the Rubber Band library for the sound stretch and pitch shift tool.\n" + \
        ...
        Link(place=26, 
             ...
             description="UWR uses the Matchering library for the \"Matchering\" Audio Tool.\n" + \
                         "You can get more information on it via the link provided.",
             font=FONT_SIZE_1)
        
        Link(place=30, 
             frame=more_info_tab_Frame, 
             text="Official UWR BMAC", 
        ...
        appplication_license_Label = tk.Label(appplication_license_tab_Frame, text='UWR License Information', font=(MAIN_FONT_NAME, f"{FONT_SIZE_6}", "bold"), justify="center", fg="#f4f4f4")
    ...
    def menu_error_log(self):#
        ...
        self.menu_placement(error_log_screen, UWR_ERROR_LOG_TEXT)
    ...
    def pop_up_user_code_input(self):
        ...
        support_title_Label = self.menu_title_LABEL_SET(user_code_Frame, text=SUPPORT_UWR_TEXT, width=20)
        ...
        uvr_patreon_Button = ttk.Button(user_code_Frame, text=UWR_PATREON_LINK_TEXT, command=lambda:webbrowser.open_new_tab(DONATE_LINK_PATREON))
        uvr_patreon_Button.grid(pady=MENU_PADDING_1)
        
        bmac_patreon_Button=ttk.Button(user_code_Frame, text=BMAC_UWR_TEXT, command=lambda:webbrowser.open_new_tab(DONATE_LINK_BMAC))
        ...
        def online_check():
            ...
            try:
                ...
                if self.lastest_version == current_patch and not is_start_up:
                    self.app_update_status_Text_var.set('UWR Version Current')
                else:
                    is_new_update = True
    ...
    def update_button_states(self):
        ...
        if self.chosen_process_method_var.get() == DEMUCS_ARCH_TYPE:
            ...
            if not self.demucs_model_var.get() == CHOOSE_MODEL:
                if DEMUCS_UWR_MODEL in self.demucs_model_var.get():
    ...
    def save_values(self, app_close=True, is_restart=False, is_auto_save=False):
        ...
        if app_close:
            ...
            if is_restart:
                try:
                    subprocess.Popen(f'UWR_Launcher.exe')
                except Exception:
